...
class QtConan(ConanFile):
    ...
    def build(self):
        ...
        args += [f"-platform linux-zig"]

        # Create custom mkspec for Zig
        mkspecs_dir = os.path.join(self.source_folder, "qt5", "qtbase", "mkspecs")
        original_mkspec = os.path.join(mkspecs_dir, "linux-clang")
        custom_mkspec = os.path.join(mkspecs_dir, "linux-zig")
        shutil.copytree(original_mkspec, custom_mkspec)

        qmake_conf = os.path.join(custom_mkspec, "qmake.conf")
        content = load(self, qmake_conf)
        content += "\nQMAKE_LFLAGS_RPATHLINK =\n"
        save(self, qmake_conf, content)
